...
Dit document is de governance-specificatie voor Level 4 goedkeuringen binnen CM_OS. De operationele audit trail wordt uitsluitend vastgelegd in het CM_APPROVAL_REGISTER in Drive; er wordt geen tweede approval-systeem naast dit register geïntroduceerd.
...
**Opslag:** Google Drive
**Pad:** `OS_CUSTOMMADE/07_LEGAL/APPROVALS/CM_APPROVAL_REGISTER`
...
1. CM CONTROL maakt een nieuwe rij aan in `CM_APPROVAL_REGISTER` met status `Open`.
...
CM CONTROL rapporteert op basis van `CM_APPROVAL_REGISTER`:
...
- Rijen verwijderen uit `CM_APPROVAL_REGISTER` — ook geweigerde en verlopen verzoeken blijven staan
...
**Stap 1:** Google Sheet aanmaken op pad `OS_CUSTOMMADE/07_LEGAL/APPROVALS/CM_APPROVAL_REGISTER`
