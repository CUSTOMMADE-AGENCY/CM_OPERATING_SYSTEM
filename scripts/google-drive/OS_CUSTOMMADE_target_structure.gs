 ...
 * - Geen verdere structuurwijzigingen. Script blijft create-only en idempotent.
 * ───────────────────────────────────────────────────────────────────────────
 *
 * Purpose:
...
const OS_CUSTOMMADE_TARGET_STRUCTURE = {
  ...
  // 2G-1: permanente intake-root. Idempotent: bestaande 00_INBOX (8 jun) wordt
  // herkend als EXISTS; bestaande children (TO-SORT, _SCHOONMAAK_TE_SORTEREN)
...
function createOsCustommadeTargetStructure() {
  ...
  try {
    ...
    addTargetStructureLogRow_(rows, rootPath, 'ROOT_FOLDER_ID', rootPath, 'ERROR', ROOT_FOLDER_ID, errorMessage);
    appendTargetStructureRows_(logSheet, rows);
    Logger.log('NO-GO / ERROR VOOR TARGET STRUCTURE DRY CHECK: ' + errorMessage);
    Logger.log('Logsheet: ' + spreadsheet.getUrl());
    throw new Error('NO-GO / ERROR VOOR TARGET STRUCTURE DRY CHECK — ' + errorMessage + ' — logsheet: ' + spreadsheet.getUrl());
  }
